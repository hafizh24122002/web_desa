    ...
    <div class="container-fostrap">
        <div class="content">
			<div class="row mt-3 container">
				<div class="col-lg">
					<form action="/staf/kesehatan/pemantauan/edit-pemantauan-ibu/{{ $ibu_hamil->id }}" method="POST">
						@method('put')
						@csrf
						...
						<div class="form-group row">
							...
							<div class="col-sm-9">
								<select class="form-select form-select-sm @error('id_kia') is-invalid @enderror" id="id_kia" name="id_kia">
									...
									@foreach ($kia as $item)
										<option value="{{ $loop->iteration }}" {{ (old('id_kia') ?? $ibu_hamil->id_kia) == $loop->iteration ? 'selected' : '' }}>{{ $item->no_kia.' - '.$item->ibu->nama }}</option>
									@endforeach
								</select>
						...
						<div class="form-group row">
							...
							<div class="col-sm-9">
								<input type="date"
									...
									value="{{ old('tanggal_periksa') ?? $ibu_hamil->tanggal_periksa->format('Y-m-d') }}">
							...
							<div class="col-sm-9">
								<select class="form-select form-select-sm @error('id_posyandu') is-invalid @enderror" id="id_posyandu" name="id_posyandu">
									...
									@foreach ($posyandu as $item)
										<option value="{{ $loop->iteration }}" {{ (old('id_posyandu') ?? $ibu_hamil->id_posyandu) == $loop->iteration ? 'selected' : '' }}>{{ $item->nama }}</option>
									@endforeach
								</select>
						...
						<div class="form-group row">
							...
							<div class="col-sm-9">
								<select class="form-select form-select-sm @error('status_kehamilan') is-invalid @enderror" id="status_kehamilan" name="status_kehamilan">
									<option value="">-- Pilih --</option>
									<option value="Normal (N)" {{ (old('status_kehamilan') ?? $ibu_hamil->status_kehamilan) == 'Normal (N)' ? 'selected' : '' }}>Normal (N)</option>
									<option value="Risiko Tinggi (Risti)" {{ (old('status_kehamilan') ?? $ibu_hamil->status_kehamilan) == 'Risiko Tinggi (Risti)' ? 'selected' : '' }}>Risiko Tinggi (Risti)</option>
									<option value="Kekurangan Energi Kronis (KEK)" {{ (old('status_kehamilan') ?? $ibu_hamil->status_kehamilan) == 'Kekurangan Energi Kronis (KEK)' ? 'selected' : '' }}>Kekurangan Energi Kronis (KEK)</option>
								</select>
						...
						<div class="form-group row">
							...
							<div class="col-sm-9">
								<input type="number"
									...
									value="{{ old('usia_kehamilan') ?? $ibu_hamil->usia_kehamilan }}">
						...
						<div class="form-group row">
							...
							<div class="col-sm-9">
								<input type="date"
									...
									value="{{ old('tanggal_melahirkan') ?? ($ibu_hamil->tanggal_melahirkan ? $ibu_hamil->tanggal_melahirkan->format('Y-m-d') : '') }}">
							</div>
						</div>
						...
						<div class="form-group row" id="butir_pil_fe">
							...
							<div class="col-sm-9">
								<input type="number"
									...
									value="{{ old('butir_pil_fe') ?? $ibu_hamil->butir_pil_fe }}">
						...
						<div class="form-group row mt-2">
							<div class="col-sm-6 d-flex gap-2">
								<input type="checkbox" name="pemeriksaan_kehamilan" id="pemeriksaan_kehamilan" {{ (old('pemeriksaan_kehamilan') ?? $ibu_hamil->pemeriksaan_kehamilan) ? 'checked' : '' }}>
								<label for="pemeriksaan_kehamilan">Pemeriksaan Kehamilan</label>
							</div>

							<div class="col-sm-6 d-flex gap-2">
								<input type="checkbox" name="konsumsi_pil_fe" id="konsumsi_pil_fe" {{ (old('konsumsi_pil_fe') ?? $ibu_hamil->konsumsi_pil_fe) ? 'checked' : '' }}>
						...
						<div class="form-group row">
							<div class="col-sm-6 d-flex gap-2">
								<input type="checkbox" name="pemeriksaan_nifas" id="pemeriksaan_nifas" {{ (old('pemeriksaan_nifas') ?? $ibu_hamil->pemeriksaan_nifas) ? 'checked' : '' }}>
								<label for="pemeriksaan_nifas">Pemeriksaan Nifas</label>
							</div>

							<div class="col-sm-6 d-flex gap-2">
								<input type="checkbox" name="konseling_gizi" id="konseling_gizi" {{ (old('konseling_gizi') ?? $ibu_hamil->konseling_gizi) ? 'checked' : '' }}>
						...
						<div class="form-group row">
							<div class="col-sm-6 d-flex gap-2">
								<input type="checkbox" name="kunjungan_rumah" id="kunjungan_rumah" {{ (old('kunjungan_rumah') ?? $ibu_hamil->kunjungan_rumah) ? 'checked' : '' }}>
								<label for="kunjungan_rumah">Kunjungan Rumah</label>
							</div>

							<div class="col-sm-6 d-flex gap-2">
								<input type="checkbox" name="akses_air_bersih" id="akses_air_bersih" {{ (old('akses_air_bersih') ?? $ibu_hamil->akses_air_bersih) ? 'checked' : '' }}>
						...
						<div class="form-group row">
							<div class="col-sm-6 d-flex gap-2">
								<input type="checkbox" name="kepemilikan_jamban" id="kepemilikan_jamban" {{ (old('kepemilikan_jamban') ?? $ibu_hamil->kepemilikan_jamban) ? 'checked' : '' }}>
								<label for="kepemilikan_jamban">Kepemilikan Jamban</label>
							</div>

							<div class="col-sm-6 d-flex gap-2">
								<input type="checkbox" name="jaminan_kesehatan" id="jaminan_kesehatan" {{ (old('jaminan_kesehatan') ?? $ibu_hamil->jaminan_kesehatan) ? 'checked' : '' }}>
						...
						<div class="d-sm-flex justify-content-md-end">
							<button class="btn btn-primary mt-2 mb-4 px-3 py-1">Ubah Data Pemantauan Ibu</button>
						</div>
					</form>
				</div>
			</div>
